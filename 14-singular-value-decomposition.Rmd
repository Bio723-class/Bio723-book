...
PCA.biplot <- function(X, center=TRUE, scale=FALSE, emphasize.variables=TRUE){
  ...
  list(scores=as_data_frame(biplot.scores),
       vectors=as_data_frame(biplot.vectors) %>% mutate(Variable = colnames(X)))
}
```
...
small.arrow = arrow(angle = 15, length = unit(0.1, "inches"))
