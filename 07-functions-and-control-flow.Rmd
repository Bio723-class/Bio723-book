...
```
Arg1 and arg2 are function arguments that allow you to adjust the values of the expressions every time you run the function. To make this concrete, here's an example where we define a function to calculate the area of a circle:
...
Function arguments can specify the data that a function operates on or parameters that the function use, i.e., the "input" or "independent varialbe" of the function. Function arguments can be either required or optional. In the case of optional arguments, a default value is assigned if the argument is not given. Any R objects can be taken as function arguments, including bare numbers, vectors, lists, data.frames, names of assigned variables, and even other functions.
