...
```{r}
spellman.clean %<>%   # note use of magritrr $<>% operator here
...
```{r}
spellman.clean %<>% 
  dplyr::select(-matches("cln3")) %>%
  dplyr::select(-matches("clb2"))
```
...
```{r}
spellman.clean %<>% 
  dplyr::select(-matches("cln3|clb2"))
```
...
```{r}
spellman.merged %<>%
  dplyr::select(gene, std.name, description, everything())
...
spellman.wide <-
  spellman.final %>%
  dplyr::select(-std.name, -description) %>% # drop unneeded columns
...
top1k.cdc28.wide <- 
  top1k.cdc28 %>%
  dplyr::select(-std.name, -description) %>%
  spread(gene, expression)
```
...
cdc28.correlations <- 
  top1k.cdc28.wide %>%
  dplyr::select(-expt, -time)  %>% # drop expt and time
